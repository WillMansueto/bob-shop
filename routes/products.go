...
var (
	ErrPriceValue = errors.New("Erro de entrada: \"Preço\" é invalido")
...
func productEditPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	sessions.Message(fmt.Sprintf("%d produto atualizado com sucesso!", rows), "info", r, w)
	http.Redirect(w, r, "/products", 302)
}
...
func productDeleteGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	sessions.Message(fmt.Sprintf("%d produtos foi excluído permanentemente!", rows), "warning", r, w)
	http.Redirect(w, r, "/products", 302)
}